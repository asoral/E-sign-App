      ...
      frappe.listview_settings[doctype] = {
        onload(listview) {
          listview.page.add_inner_button(__("Send to eSign"), async () => {
            ...
            const doctype2 = listview.doctype;
            const user = frappe.session.user;
            const userDetails = await frappe.db.get_value("User", user, ["full_name", "email"]);
            const email = ((_a = userDetails == null ? void 0 : userDetails.message) == null ? void 0 : _a.email) || "No Email";
            const fullName = ((_b = userDetails == null ? void 0 : userDetails.message) == null ? void 0 : _b.full_name) || "Unknown User";
            let userEmailList = [];
            try {
            ...
            let templates = [];
            try {
              const response = await fetch(`/api/method/esign_app.api.get_templetes_for_doctype?user_mail=${email}&requesting_doctype=${doctype2}`);
            ...
            const dialog = new frappe.ui.Dialog({
              ...
              primary_action: async (values) => {
                ...
                updatedComponentData.forEach((comp) => {
                  ...
                  comp.assign = (updated == null ? void 0 : updated.email) ? [updated.email] : [];
                });
                for (let i2 = 0; i2 < docnames.length; i2++) {
                  const docname = docnames[i2];
                  const noLetterhead = values.letterhead === "No Letterhead" ? 1 : 0;
                  const pdfUrl = `/api/method/frappe.utils.print_format.download_pdf?doctype=${doctype2}&name=${docname}&format=${values.print_format}&no_letterhead=${noLetterhead}&letterhead=${encodeURIComponent(values.letterhead)}&_lang=en`;
                  ...
                  frappe.show_progress("Sending Documents", i2 + 1, docnames.length);
                }
            ...
            dialog.fields_dict.template_select.df.onchange = async () => {
              ...
              try {
                ...
                const raw = (_a2 = response.message) == null ? void 0 : _a2.templete_json_data;
                if (raw) {
                  ...
                  assignmentTable.df.data = parsed.map((row) => {
                    ...
                    return {
                      component: row.name,
                      email: ((_a3 = row.assign) == null ? void 0 : _a3[0]) || ""
                    };
                  });
...
//# sourceMappingURL=esign.bundle.LVAR44EP.js.map
